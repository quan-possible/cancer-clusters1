...
from models.networks import (
    VGGEncoder, VGGDecoder, Encoder, Decoder, Encoder_small, Decoder_small)
...
class GMM_Survival(tf.keras.Model):
    ...
    def call(self, inputs, training=True):
        ...
        if self.survival:
            ...
            risk_scores = risk_scores_t if training else risk_scores_nt
            # risk_scores = tf.cond(
            #     use_t_value < 0.5, lambda: risk_scores_nt, lambda: risk_scores_t)
        else:
            inds = tf.dtypes.cast(tf.argmax(p_c_z, axis=-1), tf.int32)
