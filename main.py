...
def train(x, y, config, validation_data=None, log=True):
    # params
    ...
    # train
    model.fit(*get_input(x_tr, y_tr), validation_data=get_input(
        x_val, y_val), epochs=epochs, callbacks=cbs, batch_size=len(x_tr), verbose=0)
    return model, (x_tr, x_val, y_tr, y_val)


if __name__ == "__main__":
    setup_env()
    config = config_radcure()
    setup_seed(config["seed"])
    filepath = "./datasets/radcure/clinical_train.csv"

    x, y = load_radcure(filepath)
    model, history = train(x, y, config)
